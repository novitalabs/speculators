...
class Qwen3DFlashAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        # `or`-fallback: configs built from verifiers without head_dim (e.g.
        # Kimi MLA) carry head_dim=None, so plain getattr would return None.
        self.head_dim = (
            getattr(config, "head_dim", None)
            or config.hidden_size // config.num_attention_heads  # type: ignore[operator]
        )
