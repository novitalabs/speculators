...
class DFlashSpeculatorConfig(SpeculatorModelConfig):
    ...
    apply_verifier_norm: bool = Field(
        default=True,
        description=(
            "Apply the verifier's final RMSNorm to verifier_last_hidden_states "
            "before the frozen verifier_lm_head when computing distillation "
            "targets. Set False when the training data already carries "
            "post-norm hidden states (e.g. camelot rollout capture with "
            "verifier_last_hidden_states_prenorm=false), which would otherwise "
