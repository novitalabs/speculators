...
###############################################################################
# Exp 20-2: Training with continuous datagen + difficulty feedback
# Chinese subset of nemotron-v2. Aurora architecture.
# Data synced from datagen node (.17) via rsync.
###############################################################################
...
OUTPUT_PATH="${OUTPUT_PATH:-/data/output/minimax_m2.5_eagle3_exp20_2}"
NUM_GPUS="${NUM_GPUS:-8}"
...
REMOTE_GEN_DIR="/data/output/minimax_m2.5_eagle3_exp20_2/gen"
VOCAB_DIR="$OUTPUT_PATH/vocab_mapping"

echo "============================================="
echo " Exp 20-2: Chinese Subset Training"
...
torchrun \
    ...
    --run-name "minimax_m2.5_eagle3_exp20_2" \
    2>&1 | tee "$TRAIN_LOG"

kill $SYNC_PID $CLEANUP_PID $DIFFICULTY_SYNC_PID 2>/dev/null || true

echo "============================================="
echo " Exp 20-2 training complete!"
