...
###############################################################################
# Exp 14: MiniMax-M2.5 Aurora-Architecture Streaming Training
# Retrains Eagle3 draft model with Aurora-identical architecture:
#   - 24 attention heads (vs 48 in Exp13)
#   - 8192 intermediate_size (vs 1536)
#   - 32K draft vocab (vs 200064)
#   - rope_theta=5000000 (vs 10000 bug)
# Reuses Exp13 datagen data (52K files on .17/.18).
###############################################################################
...
OUTPUT_PATH="${OUTPUT_PATH:-/data/output/minimax_m2.5_eagle3_aurora_arch}"
NUM_GPUS="${NUM_GPUS:-8}"
...
REMOTE_GEN_DIR="/data/output/minimax_m2.5_eagle3_novita_full_v2/gen"
VOCAB_DIR="$OUTPUT_PATH/vocab_mapping"

echo "============================================="
echo " Exp 14: MiniMax-M2.5 Aurora-Arch Training"
...
###############################################################################
# Background: rsync from datagen node (reuse Exp13 data)
###############################################################################
...
torchrun \
    ...
    --run-name "minimax_m2.5_eagle3_aurora_arch" \
    2>&1 | tee "$TRAIN_LOG"
...
echo "============================================="
echo " Exp 14 Aurora-arch training complete!"
