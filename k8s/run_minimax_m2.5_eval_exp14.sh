...
###############################################################################
# MiniMax-M2.5 Eagle3 Evaluation - Exp14 Aurora-Architecture Checkpoints
# Compare: baseline (no spec), Aurora-Spec-M2.1, Exp14 ckpt54/60/66
# Key test: Exp14 uses Aurora-compatible arch → torch.compile should work
###############################################################################
...
EXP14_CKPT_DIR="${EXP14_CKPT_DIR:-/data/output/minimax_m2.5_eagle3_aurora_arch/checkpoints}"
OUTPUT_DIR="/data/output/minimax_m2.5_eval_exp14"
TP="${TP:-4}"
...
if __name__ == "__main__":
    evals_to_run = sys.argv[1:] if len(sys.argv) > 1 else ["baseline", "aurora_spec", "exp14_ckpt54", "exp14_ckpt60", "exp14_ckpt66"]
    ...
    for eval_name in evals_to_run:
        ...
        elif eval_name.startswith("exp14_ckpt"):
            ckpt_num = eval_name.replace("exp14_ckpt", "")
            ckpt_path = os.path.join(os.environ.get("EXP14_CKPT_DIR", "/data/output/minimax_m2.5_eagle3_aurora_arch/checkpoints"), ckpt_num)
            if not os.path.exists(ckpt_path):
                print(f"[WARN] Checkpoint not found: {ckpt_path}, skipping {eval_name}")
                continue
            # Exp14 uses Aurora-compatible architecture → torch.compile should work (no enforce_eager)
...
export AURORA_SPEC EXP14_CKPT_DIR
export NOVITA_DATA="${NOVITA_DATA:-/data/datasets/novita20260312_eval/conversations.jsonl}"

LOGFILE="$OUTPUT_DIR/eval.log"

# Run: baseline → aurora_spec → exp14 checkpoints
# Each in separate process for clean GPU memory
for eval_name in baseline aurora_spec exp14_ckpt54 exp14_ckpt60 exp14_ckpt66; do
    echo ""
...
print()
print('='*80)
print('  FINAL SUMMARY — Exp14 Eval')
print('='*80)
header = f\"{'Model':<20} {'Tokens/s':>10} {'Speedup':>8} {'Acc Len':>8} {'Acc@0':>7} {'Acc@1':>7} {'Acc@2':>7}\"
print(header)
print('-' * len(header))
