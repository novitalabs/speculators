...
###############################################################################
# MiniMax-M2.5 Eagle3 Evaluation - Exp15 novita0320 Aurora-Architecture
# Compare: baseline (no spec), Aurora-Spec-M2.1, Exp15 ckpt66/67
###############################################################################
...
OUTPUT_DIR="/data/output/minimax_m2.5_eval_exp15"
TP="${TP:-4}"
...
if __name__ == "__main__":
    evals_to_run = sys.argv[1:] if len(sys.argv) > 1 else ["baseline", "aurora_spec", "exp15_ckpt66", "exp15_ckpt67"]
    ...
    for eval_name in evals_to_run:
        if eval_name == "baseline":
            ...
            ckpt_path = os.path.join(os.environ.get("EXP15_CKPT_DIR", "/data/output/minimax_m2.5_eagle3_novita0320/checkpoints"), ckpt_num)
...
export AURORA_SPEC EXP15_CKPT_DIR
export NOVITA_DATA="${NOVITA_DATA:-/data/datasets/novita20260312_eval/conversations.jsonl}"

# Run: baseline → aurora_spec → exp15 checkpoints
for eval_name in baseline aurora_spec exp15_ckpt66 exp15_ckpt67; do
    echo ""
...
print()
print('='*80)
print('  FINAL SUMMARY — Exp15 Eval')
print('='*80)
header = f\"{'Model':<20} {'Tokens/s':>10} {'Speedup':>8} {'Acc Len':>8} {'Acc@0':>7} {'Acc@1':>7} {'Acc@2':>7}\"
print(header)
print('-' * len(header))
