...
EXP13_SPEC="${EXP13_SPEC:-/data/output/minimax_m2.5_eagle3_novita_full_v2/checkpoints/60}"
OUTPUT_DIR="/data/output/minimax_m2.5_eval_exp13"
TP="${TP:-4}"
...
if __name__ == "__main__":
    ...
    evals_to_run = sys.argv[1:] if len(sys.argv) > 1 else ["baseline", "aurora_spec", "exp13_ckpt60"]
    ...
    for eval_name in evals_to_run:
        ...
        elif eval_name == "exp13_ckpt60":
            result = run_eval("exp13_ckpt60", {
                "model": os.environ.get("EXP13_SPEC", "/data/output/minimax_m2.5_eagle3_novita_full_v2/checkpoints/60"),
                "num_speculative_tokens": 3,
                "method": "eagle3",
            }, enforce_eager=True)
        else:
...
export AURORA_SPEC EXP13_SPEC
export NOVITA_DATA="${NOVITA_DATA:-/data/datasets/novita20260309/conversations_full_no_filter.jsonl}"
...
for eval_name in baseline exp13_ckpt60 aurora_spec; do
    echo ""
