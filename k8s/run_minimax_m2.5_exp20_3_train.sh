...
###############################################################################
# Exp 20-3: Training with continuous datagen + difficulty feedback
# Dataset: nemotron-v2-chinese (195K) + novita (166K) = 361K
...
DATAGEN_NODE="${DATAGEN_NODE:-10.83.115.22}"
...
REMOTE_GEN_DIR="/data/output/minimax_m2.5_eagle3_exp20_3/gen"
VOCAB_DIR="$OUTPUT_PATH/vocab_mapping"

echo "============================================="
echo " Exp 20-3: Continuous Datagen Training (chinese+novita)"
...
torchrun \
    ...
    --keep-checkpoints 5 \
    --run-name "minimax_m2.5_eagle3_exp20_3" \
    2>&1 | tee "$TRAIN_LOG"

kill $SYNC_PID $CLEANUP_PID $DIFFICULTY_SYNC_PID 2>/dev/null || true

echo "============================================="
echo " Exp 20-3 training complete!"
