...
###############################################################################
# Exp 16: Aurora Accept/Discard Loss Training
# Same architecture + data as Exp15, but with Aurora dynamic accept/discard
# loss (--aurora-loss). A/B comparison against standard KL (Exp15).
#
# Data: Exp15's 6K .pt files, rsynced from .10
# Node: .17 (8x H200)
###############################################################################
...
OUTPUT_PATH="${OUTPUT_PATH:-/data/output/minimax_m2.5_eagle3_aurora_loss}"
NUM_GPUS="${NUM_GPUS:-8}"
...
# Aurora architecture overrides (same as Exp14/15)
...
echo "============================================="
echo " Exp 16: Aurora Accept/Discard Loss Training"
...
###############################################################################
# Step 2: Training with Aurora loss
###############################################################################
...
torchrun \
    ...
    --run-name "minimax_m2.5_eagle3_aurora_loss" \
    2>&1 | tee "$TRAIN_LOG"

echo "============================================="
echo " Exp 16 Aurora loss training complete!"
