...
###############################################################################
# MiniMax-M2.5 Eagle3 Evaluation - Exp17 novita0327 Aurora-Architecture
# Compare: baseline (no spec), Aurora-Spec-M2.1, Exp15 ckpt67, Exp17 ckpt86
#
# Designed to run INSIDE the datagen pod on .17 using GPU 4-7 (datagen uses 0-3)
# Usage: CUDA_VISIBLE_DEVICES=4,5,6,7 bash k8s/run_minimax_m2.5_eval_exp17.sh
###############################################################################
...
EXP17_CKPT_DIR="${EXP17_CKPT_DIR:-/data/output/minimax_m2.5_eagle3_novita0327/checkpoints}"
EXP15_CKPT_DIR="${EXP15_CKPT_DIR:-/data/output/minimax_m2.5_eagle3_novita0320/checkpoints}"
OUTPUT_DIR="/data/output/minimax_m2.5_eval_exp17"
TP="${TP:-4}"
...
NOVITA_DATA = os.environ.get("NOVITA_DATA", "/data/datasets/novita20260312_eval/conversations.jsonl")
...
if __name__ == "__main__":
    evals_to_run = sys.argv[1:] if len(sys.argv) > 1 else ["baseline", "aurora_spec", "exp15_ckpt67", "exp17_ckpt86"]
    ...
    for eval_name in evals_to_run:
        ...
        elif eval_name.startswith("exp15_ckpt"):
            ckpt_num = eval_name.replace("exp15_ckpt", "")
            ckpt_path = os.path.join(os.environ.get("EXP15_CKPT_DIR", ""), ckpt_num)
            if not os.path.exists(ckpt_path):
                print(f"[WARN] Checkpoint not found: {ckpt_path}, skipping {eval_name}")
                continue
        ...
        elif eval_name.startswith("exp17_ckpt"):
            ckpt_num = eval_name.replace("exp17_ckpt", "")
            ckpt_path = os.path.join(os.environ.get("EXP17_CKPT_DIR", ""), ckpt_num)
            if not os.path.exists(ckpt_path):
                print(f"[WARN] Checkpoint not found: {ckpt_path}, skipping {eval_name}")
                continue
            result = run_eval(eval_name, {
...
###############################################################################
# Copy Exp17 ckpt86 from .18 train node (if not already local)
###############################################################################
TRAIN_NODE="${DATAGEN_NODE_TRAIN:-10.83.115.18}"
EXP17_CKPT86="$EXP17_CKPT_DIR/86"

if [ ! -d "$EXP17_CKPT86" ]; then
    echo "[setup] Copying ckpt86 from $TRAIN_NODE..."
    mkdir -p "$EXP17_CKPT_DIR"
    rsync -az --progress \
        -e "ssh -o StrictHostKeyChecking=no" \
        "$TRAIN_NODE:/data/output/minimax_m2.5_eagle3_novita0327/checkpoints/86/" \
        "$EXP17_CKPT86/"
    echo "[setup] ckpt86 copied successfully"
else
    echo "[setup] ckpt86 already exists at $EXP17_CKPT86"
fi
...
export AURORA_SPEC EXP17_CKPT_DIR EXP15_CKPT_DIR
export NOVITA_DATA="${NOVITA_DATA:-/data/datasets/novita20260312_eval/conversations.jsonl}"

for eval_name in baseline aurora_spec exp15_ckpt67 exp17_ckpt86; do
    echo ""
