...
###############################################################################
# Exp 21: Training with continuous datagen + difficulty feedback
# Same Aurora architecture as exp19. Data synced from datagen node via rsync.
# Uses expanded novita_merged dataset (747K conversations, 6 sources)
###############################################################################
...
DATAGEN_NODE="${DATAGEN_NODE:-10.83.115.14}"
...
REMOTE_GEN_DIR="/data/output/minimax_m2.5_eagle3_exp21/gen"
VOCAB_DIR="$OUTPUT_PATH/vocab_mapping"

echo "============================================="
echo " Exp 21: Continuous Datagen Training"
...
torchrun \
    ...
    --run-name "minimax_m2.5_eagle3_exp21" \
    2>&1 | tee "$TRAIN_LOG"
