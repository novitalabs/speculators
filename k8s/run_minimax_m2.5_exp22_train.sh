...
###############################################################################
# Exp 22: Training — Exp21 Novita + Exp20-2 Chinese Combined
...
REMOTE_GEN_DIR="/data/output/minimax_m2.5_eagle3_exp22/gen"
VOCAB_DIR="$OUTPUT_PATH/vocab_mapping"

echo "============================================="
echo " Exp 22: Combined Novita + Chinese Training"
...
torchrun \
    ...
    --run-name "minimax_m2.5_eagle3_exp22" \
    2>&1 | tee "$TRAIN_LOG"

kill $SYNC_PID $CLEANUP_PID $DIFFICULTY_SYNC_PID 2>/dev/null || true

echo "============================================="
echo " Exp 22 training complete!"
